		...
		while(!(q1.isEmpty() && q2.isEmpty())) {
			...
			for(int i = 0 ; i < 4; i++) {
				...
				if(nx > 0 && ny > 0 && nx < m + 1 && ny < n + 1 
					&& (visit[ny][nx] == 0 || (map[ny][nx] != 1 && isBreak && visit[ny][nx] > visit[y][x] + 1))) {		// 한번 뚫었으면 다시 뚫을 수 없다.
					if(map[ny][nx] == 0) {						// 장애물이 없는 경우
						System.out.println("장애 X // nx: " + nx + " ny: " + ny + " visit["+y+"]["+x+"] " + visit[y][x]);
						q1.add(new DOT(nx, ny));
					} else if(!isBreak && map[ny][nx] == 1) {	// 장애물이 존재
						System.out.println("장애 O // nx: " + nx + " ny: " + ny + " visit["+y+"]["+x+"] " + visit[y][x]);
